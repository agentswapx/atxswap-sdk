...
项目背景与简要[团队介绍](https://docs.atxswap.com/zh/guide/team)见 ATXSwap 文档站（[English](https://docs.atxswap.com/guide/team)）。本文档仅说明 SDK 的接口与用法。
...
cd atxswap-sdk
npm install        # 通过 prepare 钩子自动构建 dist
...
| 模块 | 类 | 读/写 | 职责 |
|---|---|---|---|
| `wallet` | `WalletManager` | 读写本地文件 | 为当前技能创建 / 列出 / 加载单个钱包，并通过 `exportKeystore()` 读取加密后的 keystore V3 JSON（**不支持导入已有私钥**，也**不支持导出原始私钥**） |
...
| Linux 桌面（已设置 `DBUS_SESSION_BUS_ADDRESS`） | Secret Service / GNOME Keyring | `secret-tool` CLI |
| 其他环境（含服务器、cron、headless CI） | master key file | `aes-256-gcm` 加密的本地文件 |

Linux 上只有当 `secret-tool` 在 `PATH` 上**且** `DBUS_SESSION_BUS_ADDRESS`
被设置时，SDK 才会选择 `secret-service`，所以 cron / SSH 无桌面 / 沙箱
环境会自动降级为 file 后端，避免运行时报错。

也可以通过 `ATXSWAP_SECRET_STORE` 强制指定后端：

| 取值 | 含义 |
|---|---|
| `keychain` | macOS Keychain（依赖 `security` CLI） |
| `secret-service` | libsecret / GNOME Keyring（依赖运行中的 daemon） |
| `file` | `aes-256-gcm` 主密钥文件后端 |
| `none` | 不持久化密码；调用方每次 load 必须显式传 `password` |

后端写入失败时，`WalletManager.create()` 仍会把 keystore 写到磁盘，并返回
`{ passwordSaved: false, passwordSaveError }`；`WalletManager.load(address, password)`
则会静默忽略保存失败，因为钱包此时已经解锁完成。
...
- `create()` 成功后默认自动保存密码（可通过 `options.savePassword = false` 关闭）
